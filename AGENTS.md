...
- Install any commands the repo relies on (for example `just` or `cargo-insta`) if they aren't already available before running instructions here.
...
- Use the `execute` tool only when the user's task cannot be completed with other local tools. Prefer purpose-built local tools for reading, searching, editing, and deleting files; reach for `execute` only when a command runner is genuinely required.
